...
class ModelArgs:
    ...
    sequence_length_limit: int = 2048
...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0, use_scaled_rope: bool = True):
    if use_scaled_rope:
        theta = theta * (dim / 64)
...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.cache_k = torch.zeros(
            (args.batch_size_limit, args.sequence_length_limit, self.n_local_kv_heads, self.head_dim)
        )
...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        self.freqs_cis = precompute_freqs_cis(
            ...
            params.sequence_length_limit * 2,
            params.rope_theta,
